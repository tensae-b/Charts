import "./style.css";
import * as echarts from "echarts";
import { articleReadTimeChartOptions, ArticleTotalReadTimeChartOption, totalReadedTimeChartOption } from "./options";

let selectors = {
  articleTotalReadTimeChart: document.getElementById("readed-time-chart"),
  articleReadTimeChart: document.getElementById("article-read-time-chart"),
  readTimeChart: document.getElementById("completion-rate-for-article-chart"),
  completionRateForArticleChart: document.getElementById("completion-rate-for-article-chart"),
  interactionTypeCountWithName: document.getElementById("interaction-type-count-with-name"),
  articleInteractionScoreByAudienceType: document.getElementById("article-interaction-score-by-audience-type"),
  audienceLevelPeoplesChoicePercentages: document.getElementById("audience-level-peoples-choice-percentages"),
}

// functions
const initArticleReadTimeChart = (readTime) => {
  let chart = echarts.init(selectors.articleTotalReadTimeChart);
  let series = {
    name: 'Pressure',
    type: 'gauge',
    detail: {
      formatter: '{value}'
    },
    data: [
      {
        value: readTime,
        name: 'SCORE'
      }
    ]
  }
  chart.setOption({ ...articleReadTimeChartOptions, series });

}
const initArticleTotalReadTimeChart = (data) => {
  let chart = echarts.init(selectors.articleReadTimeChart);

  function structureData(data) {
    data = Object.entries(data)
    let tempValue = [];
    data.forEach((element, index) => {
      tempValue.push({
        value: element[1],
        name: element[0],
        title: {
          offsetCenter: ['0%', `-${index * 30}%`]
        },
        detail: {
          valueAnimation: true,
          offsetCenter: ['0%', `-${index * 20} % `]
        }
      })
    });


    return tempValue;
  }

  let structuredData = structureData(data.percentages['Total Read Completion Rate']['Total Completion rate breakdown'])

  const series = ArticleTotalReadTimeChartOption.series[0];
  chart.setOption({ ...ArticleTotalReadTimeChartOption, series: { ...series, data: structuredData } });
}

const initReadTimeChart = (data) => {
  const chart = echarts.init(selectors.readTimeChart);
  let dataa = [
    {
      value: 100,
      name: "SCORE",
    },
  ]
  const series = totalReadedTimeChartOption.series[0];
  chart.setOption({ ...totalReadedTimeChartOption, series: { ...series, data: dataa } });
}
const initCompletionRateForArticleChart = (data) => { }
const initInteractionTypeCountWithName = (data) => { }
const initArticleInteractionScoreByAudienceType = (data) => { }
const initAudienceLevelPeoplesChoicePercentages = (data) => { }



async function fetchedPostById(postId) {
  const url = `http://localhost:3000/post/${postId}`; // Construct the URL using the postId
  console.log(`Fetching post with post_id: ${postId}`);

  try {
    const response = await fetch(url);

    // Check if the response is okay
    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status}`);
    }

    const data = await response.json();
    if (data) {
      console.log(`Fetched read_time: ${data.read_time}`);
      console.log(data);
      initArticleReadTimeChart(data.read_time)
      initArticleTotalReadTimeChart(data)
      initReadTimeChart(data)
      // Update the charts with the fetched data
      // updateReadedChart(data.read_time);
      // updateArticleChartData(data); // Call without postId
      // updateCompletionRateForArticleChartData(data);
      // updateInteractionChart(data);
      // updateArticleInteractionScoreChart(data);
      // updateAudienceLevelPeoplesChoicePercentagesChart(data);
      //updateArticleEngagementPopularityChart(data);
    } else {
      console.log("Fetched data is not valid or 'read_time' is not defined.");
    }
  } catch (error) {
    console.error("Error when fetching the data:", error);
  }
}



fetchedPostById(2)
  .then((data) => {
    if (data) {
      console.log("Fetched data:", data);
    } else {
      console.log("No data fetched due to error.");
    }
  })
  .catch((error) => {
    console.error("Error:", error);
  });




// // ************ User ************
// var numberOfMPXRFromPublicationForUserChart = echarts.init(
//   document.querySelector("#number-of-MPXR-from-publication-for-user")
// );
// var numberOfMPXRFromPublicationForUserOption = {
//   tooltip: {
//     trigger: "axis",
//     axisPointer: {
//       type: "shadow",
//     },
//   },
//   legend: {},
//   grid: {
//     left: "3%",
//     right: "4%",
//     bottom: "3%",
//     containLabel: true,
//   },
//   xAxis: {
//     type: "value",
//   },
//   yAxis: {
//     type: "category",
//     data: ["MPXR Reward by Comment", "MPXR Reward from Social Post"],
//   },
//   series: [
//     {
//       name: "MPXR Reward",
//       type: "bar",
//       stack: "total",
//       label: {
//         show: true,
//         position: "inside",
//       },
//       emphasis: {
//         focus: "series",
//       },
//       itemStyle: {
//         color: function (params) {
//           let colors = ["#4CAF50", "#2196F3"]; // Green, Blue
//           return colors[params.dataIndex % colors.length]; // Alternate colors
//         },
//       },
//       data: [0, 0], // Initial data to be replaced on update
//     },
//   ],
// };

// // Set initial option
// numberOfMPXRFromPublicationForUserChart.setOption(
//   numberOfMPXRFromPublicationForUserOption
// );

// // Function to update the chart with new data
// function updateNumberOfMPXRFromPublicationForUserChart(data) {
//   if (
//     !data ||
//     !data["Reputation Score Breakdown by number"] ||
//     !data["Reputation Score Breakdown by number"]["MPXR From Comment"] ||
//     !data["Reputation Score Breakdown by number"]["MPXR From Social Post"]
//   ) {
//     console.error("Invalid data structure");
//     return;
//   }

//   const mpxrFromComment =
//     data["Reputation Score Breakdown by number"]["MPXR From Comment"];
//   const mpxrFromSocialPost =
//     data["Reputation Score Breakdown by number"]["MPXR From Social Post"];

//   // Extract relevant values
//   const mpxrRewardByComment = mpxrFromComment["MPXR Reward By Comment"] || 0; // Default to 0 if undefined
//   const mpxrRewardFromSocialPost =
//     mpxrFromSocialPost["MPXR reward for publication"] || 0; // Default to 0 if undefined

//   // Update data in the series
//   const updatedData = [mpxrRewardByComment, mpxrRewardFromSocialPost];

//   // Set the updated option
//   numberOfMPXRFromPublicationForUserChart.setOption({
//     series: [
//       {
//         name: "MPXR Reward",
//         type: "bar",
//         stack: "total",
//         data: updatedData,
//         label: {
//           show: true,
//           position: "inside",
//         },
//         emphasis: {
//           focus: "series",
//         },
//         itemStyle: {
//           color: function (params) {
//             let colors = ["#4CAF50", "#2196F3"]; // Green, Blue
//             return colors[params.dataIndex % colors.length]; // Alternate colors
//           },
//         },
//       },
//     ],
//   });
// }

// async function fetchedUserById(userId) {
//   const url = `http://localhost:3000/user/${userId}`; // Construct the URL using the postId
//   console.log(`Fetching post with post_id: ${userId}`);

//   try {
//     const response = await fetch(url);

//     // Check if the response is okay
//     if (!response.ok) {
//       throw new Error(`HTTP error! Status: ${response.status}`);
//     }
//     6;
//     const data = await response.json();
//     if (data) {
//       console.log(data);
//       updateUserCatergoricalReputationData(data);
//       updateNetworkAnalysisChart(data);
//       updateUserExpertiseLevelAndUserNatureForUserChart(data);
//       updateinteractionBreakdownInNumbersUsersFavoriteChart(data);
//       updateNumberOfMPXRFromPublicationForUserChart(data);
//       updateMPXRFromInteractionPercentageUserChart(data);
//       // Update the charts with the fetched data
//     } else {
//       console.log("Fetched data is not valid or 'read_time' is not defined.");
//     }
//   } catch (error) {
//     console.error("Error when fetching the data:", error);
//   }
// }

// fetchedUserById(2)
//   .then((data) => {
//     if (data) {
//       console.log("Fetched data:", data);
//     } else {
//       console.log("No data fetched due to error.");
//     }
//   })
//   .catch((error) => {
//     console.error("Error:", error);
//   });

// function updateReadedChart(readTime) {
//   totalReadedTimeChart.setOption({
//     series: [
//       {
//         data: [
//           {
//             value: readTime,
//             name: "SCORE",
//           },
//         ],
//       },
//     ],
//   });
// }

// // Chart Initialization
// var totalReadedTimeChart = echarts.init(
//   document.getElementById("readed-time-chart")
// );

// var totalReadedTimeChartOption = {
//   tooltip: {
//     formatter: "{a} <br/>{b} : {c}%",
//   },
//   series: [
//     {
//       name: "Pressure",
//       type: "gauge",
//       progress: {
//         show: true,
//       },
//       detail: {
//         valueAnimation: true,
//         formatter: "{value}",
//       },
//       data: [
//         {
//           value: 10,
//           name: "SCORE",
//         },
//       ],
//     },
//   ],
// };

// totalReadedTimeChart.setOption(totalReadedTimeChartOption);

// var articleTotalReadTimeChart = echarts.init(
//   document.getElementById("article-read-time-chart")
// );
// var articleTotalReadTimeChartOption;

// const gaugeData = [
//   {
//     value: 20,
//     name: "Average",
//     title: {
//       offsetCenter: ["0%", "-30%"],
//     },
//     detail: {
//       valueAnimation: true,
//       offsetCenter: ["0%", "-20%"],
//     },
//   },
//   {
//     value: 40,
//     name: "Good",
//     title: {
//       offsetCenter: ["0%", "0%"],
//     },
//     detail: {
//       valueAnimation: true,
//       offsetCenter: ["0%", "10%"],
//     },
//   },
//   {
//     value: 60,
//     name: "Best",
//     title: {
//       offsetCenter: ["0%", "30%"],
//     },
//     detail: {
//       valueAnimation: true,
//       offsetCenter: ["0%", "40%"],
//     },
//   },
// ];

// articleTotalReadTimeChartOption = {
//   series: [
//     {
//       type: "gauge",
//       startAngle: 90,
//       endAngle: -270,
//       pointer: {
//         show: false,
//       },
//       progress: {
//         show: true,
//         overlap: false,
//         roundCap: true,
//         clip: false,
//         itemStyle: {
//           borderWidth: 1,
//           borderColor: "#464646",
//         },
//       },
//       axisLine: {
//         lineStyle: {
//           width: 40,
//         },
//       },
//       splitLine: {
//         show: false,
//         distance: 0,
//         length: 10,
//       },
//       axisTick: {
//         show: false,
//       },
//       axisLabel: {
//         show: false,
//         distance: 50,
//       },
//       data: gaugeData,
//       title: {
//         fontSize: 14,
//       },
//       detail: {
//         width: 50,
//         height: 14,
//         fontSize: 14,
//         color: "inherit",
//         borderColor: "inherit",
//         borderRadius: 20,
//         borderWidth: 1,
//         formatter: "{value}%",
//       },
//     },
//   ],
// };
// articleTotalReadTimeChartOption &&
//   articleTotalReadTimeChart.setOption(articleTotalReadTimeChartOption);
// // Function to update the gauge data based on the provided post_id
// function updateArticleChartData(data) {
//   // If the object is valid, update the chart
//   if (data) {
//     const completionRate =
//       data.percentages["Total Read Completion Rate"]["Total completion_rate"];
//     const completionRateBreakdown =
//       data.percentages["Total Read Completion Rate"][
//       "Total Completion rate breakdown"
//       ];

//     // Update gauge data based on the data fetched
//     gaugeData[0].value = completionRateBreakdown.Average; // For Perfect
//     gaugeData[1].value = completionRateBreakdown.Good; // For Good
//     gaugeData[2].value = completionRateBreakdown.Best; // For Commonly

//     // Update the chart options
//     articleTotalReadTimeChart.setOption({
//       series: [
//         {
//           type: "gauge", // Specify the gauge type
//           data: gaugeData,
//         },
//       ],
//     });
//   } else {
//     console.error("Data not found.");
//   }
// }

// var completionRateForArticleChart = echarts.init(
//   document.getElementById("completion-rate-for-article-chart")
// );

// function updateCompletionRateForArticleChartData(data) {
//   console.log("Fetched Data: ", JSON.stringify(data, null, 2));

//   // Check if the necessary data exists before attempting to update charts
//   if (
//     data.percentages &&
//     data.percentages["Total Read Completion Rate"] &&
//     data.percentages["Total Read Completion Rate"][
//     "Total Audience Completion rate Breakdown"
//     ]
//   ) {
//     const breakdown =
//       data.percentages["Total Read Completion Rate"][
//       "Total Audience Completion rate Breakdown"
//       ];

//     // Prepare the chart dataset for ECharts
//     const completionRateUpdateChartOption = {
//       legend: {
//         data: [
//           "Average Audience",
//           "Intermediate Audience",
//           "Advanced Audience",
//           "Expert Audience",
//         ],
//       },
//       tooltip: {},
//       series: [
//         {
//           name: "Average Audience",
//           type: "pie",
//           radius: "20%",
//           center: ["15%", "30%"],
//           data: [
//             {
//               value: breakdown["Average Audience"].Best,
//               name: "Best",
//               itemStyle: { color: "#007bff" }, // blue for Best
//             },
//             {
//               value: breakdown["Average Audience"].Good,
//               name: "Good",
//               itemStyle: { color: "#28a745" }, // green for Good
//             },
//             {
//               value: breakdown["Average Audience"].Average,
//               name: "Average",
//               itemStyle: { color: "#ffc107" }, // yellow for Average
//             },
//           ],
//           emphasis: {
//             focus: "self",
//           },
//         },
//         {
//           name: "Intermediate Audience",
//           type: "pie",
//           radius: "20%",
//           center: ["45%", "30%"],
//           data: [
//             {
//               value: breakdown["Intermediate Audience"].Best,
//               name: "Best",
//               itemStyle: { color: "#007bff" }, // blue
//             },
//             {
//               value: breakdown["Intermediate Audience"].Good,
//               name: "Good",
//               itemStyle: { color: "#28a745" }, // green
//             },
//             {
//               value: breakdown["Intermediate Audience"].Average,
//               name: "Average",
//               itemStyle: { color: "#ffc107" }, // yellow
//             },
//           ],
//           emphasis: {
//             focus: "self",
//           },
//         },
//         {
//           name: "Advanced Audience",
//           type: "pie",
//           radius: "20%",
//           center: ["75%", "30%"],
//           data: [
//             {
//               value: breakdown["Advanced Audience"].Best,
//               name: "Best",
//               itemStyle: { color: "#007bff" }, // blue
//             },
//             {
//               value: breakdown["Advanced Audience"].Good,
//               name: "Good",
//               itemStyle: { color: "#28a745" }, // green
//             },
//             {
//               value: breakdown["Advanced Audience"].Average,
//               name: "Average",
//               itemStyle: { color: "#ffc107" }, // yellow
//             },
//           ],
//           emphasis: {
//             focus: "self",
//           },
//         },
//         {
//           name: "Expert Audience",
//           type: "pie",
//           radius: "20%",
//           center: ["15%", "75%"],
//           data: [
//             {
//               value: breakdown["Expert Audience"].Best,
//               name: "Best",
//               itemStyle: { color: "#007bff" }, // blue
//             },
//             {
//               value: breakdown["Expert Audience"].Good,
//               name: "Good",
//               itemStyle: { color: "#28a745" }, // green
//             },
//             {
//               value: breakdown["Expert Audience"].Average,
//               name: "Average",
//               itemStyle: { color: "#ffc107" }, // yellow
//             },
//           ],
//           emphasis: {
//             focus: "self",
//           },
//         },
//       ],
//     };

//     // Update the chart with the new option
//     completionRateForArticleChart.setOption(completionRateUpdateChartOption);
//   } else {
//     console.log("Completion rate data is not available in the fetched data.");
//     if (data.percentages) {
//       console.log("Percentages found in data:", data.percentages);
//     }
//     if (!data.percentages["Total Audience Completion rate"]) {
//       console.log("Total Audience Completion rate not found in percentages!");
//     }
//     if (
//       !data.percentages["Total Audience Completion rate"][
//       "Total Audience Completion rate Breakdown"
//       ]
//     ) {
//       console.log("Total Audience Completion rate Breakdown not found!");
//     }
//   }
// }

// var interactionTypeCountWithNameChart = echarts.init(
//   document.getElementById("interaction-type-count-with-name")
// );

// /*var interactionTypeCountWithNameOption = {
//   tooltip: {
//     trigger: "axis",
//     axisPointer: {
//       type: "shadow",
//     },
//   },
//   grid: {
//     left: "3%",
//     right: "4%",
//     bottom: "3%",
//     containLabel: true,
//   },
//   xAxis: [
//     {
//       type: "category",
//       data: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"],
//       axisTick: {
//         alignWithLabel: true,
//       },
//     },
//   ],
//   yAxis: [
//     {
//       type: "value",
//     },
//   ],
//   series: [
//     {
//       name: "Direct",
//       type: "bar",
//       barWidth: "60%",
//       data: [10, 52, 200, 334, 390, 330, 220],
//     },
//   ],
// };

// interactionTypeCountWithNameOption &&
//   interactionTypeCountWithNameChart.setOption(
//     interactionTypeCountWithNameOption
//   );*/

// function updateInteractionChart(data) {
//   // Extract the relevant counts from the response data
//   const emojiCount = data.percentages["Total Interaction Counts"].emoji.count;
//   const reactCount = data.percentages["Total Interaction Counts"].react.count;
//   const timeSpentCount =
//     data.percentages["Total Interaction Counts"].time_spent.count;
//   const commentCount =
//     data.percentages["Total Interaction Counts"].comment.count;

//   // Prepare the chart option
//   const interactionTypeCountWithNameOption = {
//     tooltip: {
//       trigger: "axis",
//       axisPointer: {
//         type: "shadow",
//       },
//     },
//     grid: {
//       left: "3%",
//       right: "4%",
//       bottom: "3%",
//       containLabel: true,
//     },
//     xAxis: [
//       {
//         type: "category",
//         data: ["Emoji", "React", "Time Spent", "Comment"], // Categories for the x-axis
//         axisTick: {
//           alignWithLabel: true,
//         },
//       },
//     ],
//     yAxis: [
//       {
//         type: "value",
//       },
//     ],
//     series: [
//       {
//         name: "Interactions",
//         type: "bar",
//         barWidth: "60%",
//         data: [emojiCount, reactCount, timeSpentCount, commentCount], // Data for the chart
//       },
//     ],
//   };

//   // Set the updated option to the chart
//   interactionTypeCountWithNameChart.setOption(
//     interactionTypeCountWithNameOption
//   );
// }

// var ArticleInteractionScoreByAudienceTypeChart = echarts.init(
//   document.getElementById("article-interaction-score-by-audience-type")
// );

// function updateArticleInteractionScoreChart(data) {
//   const piePatternSrc =
//     "data:image/jpeg;base64,";
//   const bgPatternSrc =
//     "data:image/png;base64,";
//   const piePatternImg = new Image();
//   piePatternImg.src = piePatternSrc;
//   const bgPatternImg = new Image();
//   bgPatternImg.src = bgPatternSrc;
//   // Extracting the values from the provided data object
//   const audienceLevelData =
//     data.percentages["Audience Level interaction percentages"];
//   const chartData = [
//     { value: audienceLevelData["Average Audience"], name: "Average Audience" },
//     {
//       value: audienceLevelData["Intermediate Audience"],
//       name: "Intermediate Audience",
//     },
//     {
//       value: audienceLevelData["Advanced Audience"],
//       name: "Advanced Audience",
//     },
//     { value: audienceLevelData["Expert Audience"], name: "Expert Audience" },
//   ];

//   // Setting new options for the chart
//   const ArticleInteractionScoreByAudienceTypeOption = {
//     backgroundColor: {
//       image: bgPatternImg,
//       repeat: "repeat",
//     },
//     title: {
//       text: "Audience Interaction Scores",
//       textStyle: {
//         color: "#235894",
//       },
//     },
//     tooltip: {
//       trigger: "item",
//     },
//     series: [
//       {
//         name: "Audience Interaction",
//         type: "pie",
//         selectedMode: "single",
//         selectedOffset: 30,
//         clockwise: true,
//         label: {
//           fontSize: 18,
//           color: "#235894",
//         },
//         labelLine: {
//           lineStyle: {
//             color: "#235894",
//           },
//         },
//         data: chartData,
//         itemStyle: {
//           opacity: 0.7,
//           color: {
//             image: piePatternImg,
//             repeat: "repeat",
//           },
//           borderWidth: 3,
//           borderColor: "#235894",
//         },
//       },
//     ],
//   };

//   // Update the chart with the new options
//   ArticleInteractionScoreByAudienceTypeChart.setOption(
//     ArticleInteractionScoreByAudienceTypeOption
//   );
// }

// var audienceLevelPeoplesChoicePercentagesChart = echarts.init(
//   document.getElementById("audience-level-peoples-choice-percentages")
// );

// var audienceLevelPeoplesChoicePercentagesOption;
// const symbols = [
//   "path://M36.7,102.84c-1.17,2.54-2.99,4.98-3.39,7.63c-1.51,9.89-3.31,19.58-1.93,29.95 c0.95,7.15-2.91,14.82-3.57,22.35c-0.64,7.36-0.2,14.86,0.35,22.25c0.12,1.68,2.66,3.17,4.67,5.4c-0.6,0.82-1.5,2.22-2.58,3.48 c-0.96,1.12-1.96,2.35-3.21,3.04c-1.71,0.95-3.71,2.03-5.51,1.9c-1.18-0.08-3.04-2.13-3.16-3.43c-0.44-4.72,0-9.52-0.41-14.25 c-0.94-10.88-2.32-21.72-3.24-32.61c-0.49-5.84-1.63-12.01-0.35-17.54c3.39-14.56,2.8-28.84,0.36-43.4 c-2.71-16.16-1.06-32.4,0.54-48.59c0.91-9.22,4.62-17.36,8.53-25.57c1.32-2.77,1.88-6.84,0.87-9.62C21.89-3.77,18.09-11,14.7-18.38 c-0.56,0.1-1.13,0.21-1.69,0.31C10.17-11.52,6.29-5.2,4.71,1.65C2.05,13.21-4.42,22.3-11.43,31.28c-1.32,1.69-2.51,3.5-3.98,5.04 c-4.85,5.08-3.25,10.98-2.32,16.82c0.25,1.53,0.52,3.06,0.77,4.59c-0.53,0.22-1.07,0.43-1.6,0.65c-1.07-2.09-2.14-4.19-3.28-6.44 c-6.39,2.91-2.67,9.6-5.23,15.16c-1.61-3.31-2.77-5.68-3.93-8.06c0-0.33,0-0.67,0-1c6.96-16.08,14.63-31.9,20.68-48.31 C-5.24-4.07-2.03-18.55,2-32.73c0.36-1.27,0.75-2.53,0.98-3.82c1.36-7.75,4.19-10.23,11.88-10.38c1.76-0.04,3.52-0.21,5.76-0.35 c-0.55-3.95-1.21-7.3-1.45-10.68c-0.61-8.67,0.77-16.69,7.39-23.19c2.18-2.14,4.27-4.82,5.25-7.65c2.39-6.88,11.66-9,16.94-8.12 c5.92,0.99,12.15,7.93,12.16,14.12c0.01,9.89-5.19,17.26-12.24,23.68c-2.17,1.97-5.35,4.77-5.17,6.94c0.31,3.78,4.15,5.66,8.08,6.04 c1.82,0.18,3.7,0.37,5.49,0.1c5.62-0.85,8.8,2.17,10.85,6.73C73.38-27.19,78.46-14.9,84.2-2.91c1.52,3.17,4.52,5.91,7.41,8.09 c7.64,5.77,15.57,11.16,23.45,16.61c2.28,1.58,4.64,3.23,7.21,4.14c5.18,1.84,8.09,5.63,9.82,10.46c0.45,1.24,0.19,3.71-0.6,4.18 c-1.06,0.63-3.15,0.27-4.44-0.38c-7.05-3.54-12.84-8.88-19.14-13.5c-3.5-2.57-7.9-4-12.03-5.6c-9.44-3.66-17.73-8.42-22.5-18.09 c-2.43-4.94-6.09-9.27-9.69-14.61c-1.2,10.98-4.46,20.65,1.14,31.19c6.62,12.47,5.89,26.25,1.21,39.49 c-2.52,7.11-6.5,13.74-8.67,20.94c-1.91,6.33-2.2,13.15-3.23,19.75c-0.72,4.63-0.84,9.48-2.36,13.84 c-2.49,7.16-6.67,13.83-5.84,21.82c0.42,4.02,1.29,7.99,2.1,12.8c-3.74-0.49-7.47-0.4-10.67-1.66c-1.33-0.53-2.43-4.11-2.07-6.01 c1.86-9.94,3.89-19.69,0.07-29.74C34.55,108.63,36.19,105.52,36.7,102.84c1.25-8.45,2.51-16.89,3.71-24.9 c-0.83-0.58-0.85-0.59-0.87-0.61c-0.03,0.16-0.07,0.32-0.09,0.48C38.53,86.15,37.62,94.5,36.7,102.84z",
//   "path://M36.7,102.84c-1.17,2.54-2.99,4.98-3.39,7.63c-1.51,9.89-3.31,19.58-1.93,29.95 c0.95,7.15-2.91,14.82-3.57,22.35c-0.64,7.36-0.2,14.86,0.35,22.25c0.12,1.68,2.66,3.17,4.67,5.4c-0.6,0.82-1.5,2.22-2.58,3.48 c-0.96,1.12-1.96,2.35-3.21,3.04c-1.71,0.95-3.71,2.03-5.51,1.9c-1.18-0.08-3.04-2.13-3.16-3.43c-0.44-4.72,0-9.52-0.41-14.25 c-0.94-10.88-2.32-21.72-3.24-32.61c-0.49-5.84-1.63-12.01-0.35-17.54c3.39-14.56,2.8-28.84,0.36-43.4 c-2.71-16.16-1.06-32.4,0.54-48.59c0.91-9.22,4.62-17.36,8.53-25.57c1.32-2.77,1.88-6.84,0.87-9.62C21.89-3.77,18.09-11,14.7-18.38 c-0.56,0.1-1.13,0.21-1.69,0.31C10.17-11.52,6.29-5.2,4.71,1.65C2.05,13.21-4.42,22.3-11.43,31.28c-1.32,1.69-2.51,3.5-3.98,5.04 c-4.85,5.08-3.25,10.98-2.32,16.82c0.25,1.53,0.52,3.06,0.77,4.59c-0.53,0.22-1.07,0.43-1.6,0.65c-1.07-2.09-2.14-4.19-3.28-6.44 c-6.39,2.91-2.67,9.6-5.23,15.16c-1.61-3.31-2.77-5.68-3.93-8.06c0-0.33,0-0.67,0-1c6.96-16.08,14.63-31.9,20.68-48.31 C-5.24-4.07-2.03-18.55,2-32.73c0.36-1.27,0.75-2.53,0.98-3.82c1.36-7.75,4.19-10.23,11.88-10.38c1.76-0.04,3.52-0.21,5.76-0.35 c-0.55-3.95-1.21-7.3-1.45-10.68c-0.61-8.67,0.77-16.69,7.39-23.19c2.18-2.14,4.27-4.82,5.25-7.65c2.39-6.88,11.66-9,16.94-8.12 c5.92,0.99,12.15,7.93,12.16,14.12c0.01,9.89-5.19,17.26-12.24,23.68c-2.17,1.97-5.35,4.77-5.17,6.94c0.31,3.78,4.15,5.66,8.08,6.04 c1.82,0.18,3.7,0.37,5.49,0.1c5.62-0.85,8.8,2.17,10.85,6.73C73.38-27.19,78.46-14.9,84.2-2.91c1.52,3.17,4.52,5.91,7.41,8.09 c7.64,5.77,15.57,11.16,23.45,16.61c2.28,1.58,4.64,3.23,7.21,4.14c5.18,1.84,8.09,5.63,9.82,10.46c0.45,1.24,0.19,3.71-0.6,4.18 c-1.06,0.63-3.15,0.27-4.44-0.38c-7.05-3.54-12.84-8.88-19.14-13.5c-3.5-2.57-7.9-4-12.03-5.6c-9.44-3.66-17.73-8.42-22.5-18.09 c-2.43-4.94-6.09-9.27-9.69-14.61c-1.2,10.98-4.46,20.65,1.14,31.19c6.62,12.47,5.89,26.25,1.21,39.49 c-2.52,7.11-6.5,13.74-8.67,20.94c-1.91,6.33-2.2,13.15-3.23,19.75c-0.72,4.63-0.84,9.48-2.36,13.84 c-2.49,7.16-6.67,13.83-5.84,21.82c0.42,4.02,1.29,7.99,2.1,12.8c-3.74-0.49-7.47-0.4-10.67-1.66c-1.33-0.53-2.43-4.11-2.07-6.01 c1.86-9.94,3.89-19.69,0.07-29.74C34.55,108.63,36.19,105.52,36.7,102.84c1.25-8.45,2.51-16.89,3.71-24.9 c-0.83-0.58-0.85-0.59-0.87-0.61c-0.03,0.16-0.07,0.32-0.09,0.48C38.53,86.15,37.62,94.5,36.7,102.84z",
//   "path://M36.7,102.84c-1.17,2.54-2.99,4.98-3.39,7.63c-1.51,9.89-3.31,19.58-1.93,29.95 c0.95,7.15-2.91,14.82-3.57,22.35c-0.64,7.36-0.2,14.86,0.35,22.25c0.12,1.68,2.66,3.17,4.67,5.4c-0.6,0.82-1.5,2.22-2.58,3.48 c-0.96,1.12-1.96,2.35-3.21,3.04c-1.71,0.95-3.71,2.03-5.51,1.9c-1.18-0.08-3.04-2.13-3.16-3.43c-0.44-4.72,0-9.52-0.41-14.25 c-0.94-10.88-2.32-21.72-3.24-32.61c-0.49-5.84-1.63-12.01-0.35-17.54c3.39-14.56,2.8-28.84,0.36-43.4 c-2.71-16.16-1.06-32.4,0.54-48.59c0.91-9.22,4.62-17.36,8.53-25.57c1.32-2.77,1.88-6.84,0.87-9.62C21.89-3.77,18.09-11,14.7-18.38 c-0.56,0.1-1.13,0.21-1.69,0.31C10.17-11.52,6.29-5.2,4.71,1.65C2.05,13.21-4.42,22.3-11.43,31.28c-1.32,1.69-2.51,3.5-3.98,5.04 c-4.85,5.08-3.25,10.98-2.32,16.82c0.25,1.53,0.52,3.06,0.77,4.59c-0.53,0.22-1.07,0.43-1.6,0.65c-1.07-2.09-2.14-4.19-3.28-6.44 c-6.39,2.91-2.67,9.6-5.23,15.16c-1.61-3.31-2.77-5.68-3.93-8.06c0-0.33,0-0.67,0-1c6.96-16.08,14.63-31.9,20.68-48.31 C-5.24-4.07-2.03-18.55,2-32.73c0.36-1.27,0.75-2.53,0.98-3.82c1.36-7.75,4.19-10.23,11.88-10.38c1.76-0.04,3.52-0.21,5.76-0.35 c-0.55-3.95-1.21-7.3-1.45-10.68c-0.61-8.67,0.77-16.69,7.39-23.19c2.18-2.14,4.27-4.82,5.25-7.65c2.39-6.88,11.66-9,16.94-8.12 c5.92,0.99,12.15,7.93,12.16,14.12c0.01,9.89-5.19,17.26-12.24,23.68c-2.17,1.97-5.35,4.77-5.17,6.94c0.31,3.78,4.15,5.66,8.08,6.04 c1.82,0.18,3.7,0.37,5.49,0.1c5.62-0.85,8.8,2.17,10.85,6.73C73.38-27.19,78.46-14.9,84.2-2.91c1.52,3.17,4.52,5.91,7.41,8.09 c7.64,5.77,15.57,11.16,23.45,16.61c2.28,1.58,4.64,3.23,7.21,4.14c5.18,1.84,8.09,5.63,9.82,10.46c0.45,1.24,0.19,3.71-0.6,4.18 c-1.06,0.63-3.15,0.27-4.44-0.38c-7.05-3.54-12.84-8.88-19.14-13.5c-3.5-2.57-7.9-4-12.03-5.6c-9.44-3.66-17.73-8.42-22.5-18.09 c-2.43-4.94-6.09-9.27-9.69-14.61c-1.2,10.98-4.46,20.65,1.14,31.19c6.62,12.47,5.89,26.25,1.21,39.49 c-2.52,7.11-6.5,13.74-8.67,20.94c-1.91,6.33-2.2,13.15-3.23,19.75c-0.72,4.63-0.84,9.48-2.36,13.84 c-2.49,7.16-6.67,13.83-5.84,21.82c0.42,4.02,1.29,7.99,2.1,12.8c-3.74-0.49-7.47-0.4-10.67-1.66c-1.33-0.53-2.43-4.11-2.07-6.01 c1.86-9.94,3.89-19.69,0.07-29.74C34.55,108.63,36.19,105.52,36.7,102.84c1.25-8.45,2.51-16.89,3.71-24.9 c-0.83-0.58-0.85-0.59-0.87-0.61c-0.03,0.16-0.07,0.32-0.09,0.48C38.53,86.15,37.62,94.5,36.7,102.84z",
//   "path://M36.7,102.84c-1.17,2.54-2.99,4.98-3.39,7.63c-1.51,9.89-3.31,19.58-1.93,29.95 c0.95,7.15-2.91,14.82-3.57,22.35c-0.64,7.36-0.2,14.86,0.35,22.25c0.12,1.68,2.66,3.17,4.67,5.4c-0.6,0.82-1.5,2.22-2.58,3.48 c-0.96,1.12-1.96,2.35-3.21,3.04c-1.71,0.95-3.71,2.03-5.51,1.9c-1.18-0.08-3.04-2.13-3.16-3.43c-0.44-4.72,0-9.52-0.41-14.25 c-0.94-10.88-2.32-21.72-3.24-32.61c-0.49-5.84-1.63-12.01-0.35-17.54c3.39-14.56,2.8-28.84,0.36-43.4 c-2.71-16.16-1.06-32.4,0.54-48.59c0.91-9.22,4.62-17.36,8.53-25.57c1.32-2.77,1.88-6.84,0.87-9.62C21.89-3.77,18.09-11,14.7-18.38 c-0.56,0.1-1.13,0.21-1.69,0.31C10.17-11.52,6.29-5.2,4.71,1.65C2.05,13.21-4.42,22.3-11.43,31.28c-1.32,1.69-2.51,3.5-3.98,5.04 c-4.85,5.08-3.25,10.98-2.32,16.82c0.25,1.53,0.52,3.06,0.77,4.59c-0.53,0.22-1.07,0.43-1.6,0.65c-1.07-2.09-2.14-4.19-3.28-6.44 c-6.39,2.91-2.67,9.6-5.23,15.16c-1.61-3.31-2.77-5.68-3.93-8.06c0-0.33,0-0.67,0-1c6.96-16.08,14.63-31.9,20.68-48.31 C-5.24-4.07-2.03-18.55,2-32.73c0.36-1.27,0.75-2.53,0.98-3.82c1.36-7.75,4.19-10.23,11.88-10.38c1.76-0.04,3.52-0.21,5.76-0.35 c-0.55-3.95-1.21-7.3-1.45-10.68c-0.61-8.67,0.77-16.69,7.39-23.19c2.18-2.14,4.27-4.82,5.25-7.65c2.39-6.88,11.66-9,16.94-8.12 c5.92,0.99,12.15,7.93,12.16,14.12c0.01,9.89-5.19,17.26-12.24,23.68c-2.17,1.97-5.35,4.77-5.17,6.94c0.31,3.78,4.15,5.66,8.08,6.04 c1.82,0.18,3.7,0.37,5.49,0.1c5.62-0.85,8.8,2.17,10.85,6.73C73.38-27.19,78.46-14.9,84.2-2.91c1.52,3.17,4.52,5.91,7.41,8.09 c7.64,5.77,15.57,11.16,23.45,16.61c2.28,1.58,4.64,3.23,7.21,4.14c5.18,1.84,8.09,5.63,9.82,10.46c0.45,1.24,0.19,3.71-0.6,4.18 c-1.06,0.63-3.15,0.27-4.44-0.38c-7.05-3.54-12.84-8.88-19.14-13.5c-3.5-2.57-7.9-4-12.03-5.6c-9.44-3.66-17.73-8.42-22.5-18.09 c-2.43-4.94-6.09-9.27-9.69-14.61c-1.2,10.98-4.46,20.65,1.14,31.19c6.62,12.47,5.89,26.25,1.21,39.49 c-2.52,7.11-6.5,13.74-8.67,20.94c-1.91,6.33-2.2,13.15-3.23,19.75c-0.72,4.63-0.84,9.48-2.36,13.84 c-2.49,7.16-6.67,13.83-5.84,21.82c0.42,4.02,1.29,7.99,2.1,12.8c-3.74-0.49-7.47-0.4-10.67-1.66c-1.33-0.53-2.43-4.11-2.07-6.01 c1.86-9.94,3.89-19.69,0.07-29.74C34.55,108.63,36.19,105.52,36.7,102.84c1.25-8.45,2.51-16.89,3.71-24.9 c-0.83-0.58-0.85-0.59-0.87-0.61c-0.03,0.16-0.07,0.32-0.09,0.48C38.53,86.15,37.62,94.5,36.7,102.84z",
//   "path://M36.7,102.84c-1.17,2.54-2.99,4.98-3.39,7.63c-1.51,9.89-3.31,19.58-1.93,29.95 c0.95,7.15-2.91,14.82-3.57,22.35c-0.64,7.36-0.2,14.86,0.35,22.25c0.12,1.68,2.66,3.17,4.67,5.4c-0.6,0.82-1.5,2.22-2.58,3.48 c-0.96,1.12-1.96,2.35-3.21,3.04c-1.71,0.95-3.71,2.03-5.51,1.9c-1.18-0.08-3.04-2.13-3.16-3.43c-0.44-4.72,0-9.52-0.41-14.25 c-0.94-10.88-2.32-21.72-3.24-32.61c-0.49-5.84-1.63-12.01-0.35-17.54c3.39-14.56,2.8-28.84,0.36-43.4 c-2.71-16.16-1.06-32.4,0.54-48.59c0.91-9.22,4.62-17.36,8.53-25.57c1.32-2.77,1.88-6.84,0.87-9.62C21.89-3.77,18.09-11,14.7-18.38 c-0.56,0.1-1.13,0.21-1.69,0.31C10.17-11.52,6.29-5.2,4.71,1.65C2.05,13.21-4.42,22.3-11.43,31.28c-1.32,1.69-2.51,3.5-3.98,5.04 c-4.85,5.08-3.25,10.98-2.32,16.82c0.25,1.53,0.52,3.06,0.77,4.59c-0.53,0.22-1.07,0.43-1.6,0.65c-1.07-2.09-2.14-4.19-3.28-6.44 c-6.39,2.91-2.67,9.6-5.23,15.16c-1.61-3.31-2.77-5.68-3.93-8.06c0-0.33,0-0.67,0-1c6.96-16.08,14.63-31.9,20.68-48.31 C-5.24-4.07-2.03-18.55,2-32.73c0.36-1.27,0.75-2.53,0.98-3.82c1.36-7.75,4.19-10.23,11.88-10.38c1.76-0.04,3.52-0.21,5.76-0.35 c-0.55-3.95-1.21-7.3-1.45-10.68c-0.61-8.67,0.77-16.69,7.39-23.19c2.18-2.14,4.27-4.82,5.25-7.65c2.39-6.88,11.66-9,16.94-8.12 c5.92,0.99,12.15,7.93,12.16,14.12c0.01,9.89-5.19,17.26-12.24,23.68c-2.17,1.97-5.35,4.77-5.17,6.94c0.31,3.78,4.15,5.66,8.08,6.04 c1.82,0.18,3.7,0.37,5.49,0.1c5.62-0.85,8.8,2.17,10.85,6.73C73.38-27.19,78.46-14.9,84.2-2.91c1.52,3.17,4.52,5.91,7.41,8.09 c7.64,5.77,15.57,11.16,23.45,16.61c2.28,1.58,4.64,3.23,7.21,4.14c5.18,1.84,8.09,5.63,9.82,10.46c0.45,1.24,0.19,3.71-0.6,4.18 c-1.06,0.63-3.15,0.27-4.44-0.38c-7.05-3.54-12.84-8.88-19.14-13.5c-3.5-2.57-7.9-4-12.03-5.6c-9.44-3.66-17.73-8.42-22.5-18.09 c-2.43-4.94-6.09-9.27-9.69-14.61c-1.2,10.98-4.46,20.65,1.14,31.19c6.62,12.47,5.89,26.25,1.21,39.49 c-2.52,7.11-6.5,13.74-8.67,20.94c-1.91,6.33-2.2,13.15-3.23,19.75c-0.72,4.63-0.84,9.48-2.36,13.84 c-2.49,7.16-6.67,13.83-5.84,21.82c0.42,4.02,1.29,7.99,2.1,12.8c-3.74-0.49-7.47-0.4-10.67-1.66c-1.33-0.53-2.43-4.11-2.07-6.01 c1.86-9.94,3.89-19.69,0.07-29.74C34.55,108.63,36.19,105.52,36.7,102.84c1.25-8.45,2.51-16.89,3.71-24.9 c-0.83-0.58-0.85-0.59-0.87-0.61c-0.03,0.16-0.07,0.32-0.09,0.48C38.53,86.15,37.62,94.5,36.7,102.84z",
// ];
// const bodyMax = 150;
// const labelSetting = {
//   show: true,
//   position: "top",
//   offset: [0, -20],
//   formatter: function (param) {
//     return ((param.value / bodyMax) * 100).toFixed(0) + "%";
//   },
//   fontSize: 18,
//   fontFamily: "Arial",
// };
// const markLineSetting = {
//   symbol: "none",
//   lineStyle: {
//     opacity: 0.3,
//   },
//   data: [
//     {
//       type: "max",
//       label: {
//         formatter: "max: {c}",
//       },
//     },
//     {
//       type: "min",
//       label: {
//         formatter: "min: {c}",
//       },
//     },
//   ],
// };
// /*audienceLevelPeoplesChoicePercentagesOption = {
//   tooltip: {},
//   legend: {
//     data: ["typeA", "typeB"],
//     selectedMode: "single",
//   },
//   xAxis: {
//     data: ["a", "b", "c", "d", "e"],
//     axisTick: { show: false },
//     axisLine: { show: false },
//     axisLabel: { show: false },
//   },
//   yAxis: {
//     max: bodyMax,
//     offset: 20,
//     splitLine: { show: false },
//   },
//   grid: {
//     top: "center",
//     height: 230,
//   },
//   markLine: {
//     z: -100,
//   },
//   series: [
//     {
//       name: "typeA",
//       type: "pictorialBar",
//       symbolClip: true,
//       symbolBoundingData: bodyMax,
//       label: labelSetting,
//       data: [
//         {
//           value: 123,
//           symbol: symbols[0],
//         },
//         {
//           value: 34,
//           symbol: symbols[1],
//         },
//         {
//           value: 101,
//           symbol: symbols[2],
//         },
//         {
//           value: 89,
//           symbol: symbols[3],
//         },
//         {
//           value: 72,
//           symbol: symbols[4],
//         },
//       ],
//       markLine: markLineSetting,
//       z: 10,
//     },
//     {
//       name: "typeB",
//       type: "pictorialBar",
//       symbolClip: true,
//       symbolBoundingData: bodyMax,
//       label: labelSetting,
//       data: [
//         {
//           value: 12,
//           symbol: symbols[0],
//         },
//         {
//           value: 44,
//           symbol: symbols[1],
//         },
//         {
//           value: 131,
//           symbol: symbols[2],
//         },
//         {
//           value: 33,
//           symbol: symbols[3],
//         },
//         {
//           value: 142,
//           symbol: symbols[4],
//         },
//       ],
//       markLine: markLineSetting,
//       z: 10,
//     },
//     {
//       name: "full",
//       type: "pictorialBar",
//       symbolBoundingData: bodyMax,
//       animationDuration: 0,
//       itemStyle: {
//         color: "#ccc",
//       },
//       data: [
//         {
//           value: 1,
//           symbol: symbols[0],
//         },
//         {
//           value: 1,
//           symbol: symbols[1],
//         },
//         {
//           value: 1,
//           symbol: symbols[2],
//         },
//         {
//           value: 1,
//           symbol: symbols[3],
//         },
//         {
//           value: 1,
//           symbol: symbols[4],
//         },
//       ],
//     },
//   ],
// };

// audienceLevelPeoplesChoicePercentagesOption &&
//   audienceLevelPeoplesChoicePercentagesChart.setOption(
//     audienceLevelPeoplesChoicePercentagesOption
//   );*/

// function updateAudienceLevelPeoplesChoicePercentagesChart(data) {
//   const symbols = [
//     "path://M36.7,102.84c-1.17,2.54-2.99,4.98-3.39,7.63c-1.51,9.89-3.31,19.58-1.93,29.95 c0.95,7.15-2.91,14.82-3.57,22.35c-0.64,7.36-0.2,14.86,0.35,22.25c0.12,1.68,2.66,3.17,4.67,5.4c-0.6,0.82-1.5,2.22-2.58,3.48 c-0.96,1.12-1.96,2.35-3.21,3.04c-1.71,0.95-3.71,2.03-5.51,1.9c-1.18-0.08-3.04-2.13-3.16-3.43c-0.44-4.72,0-9.52-0.41-14.25 c-0.94-10.88-2.32-21.72-3.24-32.61c-0.49-5.84-1.63-12.01-0.35-17.54c3.39-14.56,2.8-28.84,0.36-43.4 c-2.71-16.16-1.06-32.4,0.54-48.59c0.91-9.22,4.62-17.36,8.53-25.57c1.32-2.77,1.88-6.84,0.87-9.62C21.89-3.77,18.09-11,14.7-18.38 c-0.56,0.1-1.13,0.21-1.69,0.31C10.17-11.52,6.29-5.2,4.71,1.65C2.05,13.21-4.42,22.3-11.43,31.28c-1.32,1.69-2.51,3.5-3.98,5.04 c-4.85,5.08-3.25,10.98-2.32,16.82c0.25,1.53,0.52,3.06,0.77,4.59c-0.53,0.22-1.07,0.43-1.6,0.65c-1.07-2.09-2.14-4.19-3.28-6.44 c-6.39,2.91-2.67,9.6-5.23,15.16c-1.61-3.31-2.77-5.68-3.93-8.06c0-0.33,0-0.67,0-1c6.96-16.08,14.63-31.9,20.68-48.31 C-5.24-4.07-2.03-18.55,2-32.73c0.36-1.27,0.75-2.53,0.98-3.82c1.36-7.75,4.19-10.23,11.88-10.38c1.76-0.04,3.52-0.21,5.76-0.35 c-0.55-3.95-1.21-7.3-1.45-10.68c-0.61-8.67,0.77-16.69,7.39-23.19c2.18-2.14,4.27-4.82,5.25-7.65c2.39-6.88,11.66-9,16.94-8.12 c5.92,0.99,12.15,7.93,12.16,14.12c0.01,9.89-5.19,17.26-12.24,23.68c-2.17,1.97-5.35,4.77-5.17,6.94c0.31,3.78,4.15,5.66,8.08,6.04 c1.82,0.18,3.7,0.37,5.49,0.1c5.62-0.85,8.8,2.17,10.85,6.73C73.38-27.19,78.46-14.9,84.2-2.91c1.52,3.17,4.52,5.91,7.41,8.09 c7.64,5.77,15.57,11.16,23.45,16.61c2.28,1.58,4.64,3.23,7.21,4.14c5.18,1.84,8.09,5.63,9.82,10.46c0.45,1.24,0.19,3.71-0.6,4.18 c-1.06,0.63-3.15,0.27-4.44-0.38c-7.05-3.54-12.84-8.88-19.14-13.5c-3.5-2.57-7.9-4-12.03-5.6c-9.44-3.66-17.73-8.42-22.5-18.09 c-2.43-4.94-6.09-9.27-9.69-14.61c-1.2,10.98-4.46,20.65,1.14,31.19c6.62,12.47,5.89,26.25,1.21,39.49 c-2.52,7.11-6.5,13.74-8.67,20.94c-1.91,6.33-2.2,13.15-3.23,19.75c-0.72,4.63-0.84,9.48-2.36,13.84 c-2.49,7.16-6.67,13.83-5.84,21.82c0.42,4.02,1.29,7.99,2.1,12.8c-3.74-0.49-7.47-0.4-10.67-1.66c-1.33-0.53-2.43-4.11-2.07-6.01 c1.86-9.94,3.89-19.69,0.07-29.74C34.55,108.63,36.19,105.52,36.7,102.84c1.25-8.45,2.51-16.89,3.71-24.9 c-0.83-0.58-0.85-0.59-0.87-0.61c-0.03,0.16-0.07,0.32-0.09,0.48C38.53,86.15,37.62,94.5,36.7,102.84z",
//     "path://M36.7,102.84c-1.17,2.54-2.99,4.98-3.39,7.63c-1.51,9.89-3.31,19.58-1.93,29.95 c0.95,7.15-2.91,14.82-3.57,22.35c-0.64,7.36-0.2,14.86,0.35,22.25c0.12,1.68,2.66,3.17,4.67,5.4c-0.6,0.82-1.5,2.22-2.58,3.48 c-0.96,1.12-1.96,2.35-3.21,3.04c-1.71,0.95-3.71,2.03-5.51,1.9c-1.18-0.08-3.04-2.13-3.16-3.43c-0.44-4.72,0-9.52-0.41-14.25 c-0.94-10.88-2.32-21.72-3.24-32.61c-0.49-5.84-1.63-12.01-0.35-17.54c3.39-14.56,2.8-28.84,0.36-43.4 c-2.71-16.16-1.06-32.4,0.54-48.59c0.91-9.22,4.62-17.36,8.53-25.57c1.32-2.77,1.88-6.84,0.87-9.62C21.89-3.77,18.09-11,14.7-18.38 c-0.56,0.1-1.13,0.21-1.69,0.31C10.17-11.52,6.29-5.2,4.71,1.65C2.05,13.21-4.42,22.3-11.43,31.28c-1.32,1.69-2.51,3.5-3.98,5.04 c-4.85,5.08-3.25,10.98-2.32,16.82c0.25,1.53,0.52,3.06,0.77,4.59c-0.53,0.22-1.07,0.43-1.6,0.65c-1.07-2.09-2.14-4.19-3.28-6.44 c-6.39,2.91-2.67,9.6-5.23,15.16c-1.61-3.31-2.77-5.68-3.93-8.06c0-0.33,0-0.67,0-1c6.96-16.08,14.63-31.9,20.68-48.31 C-5.24-4.07-2.03-18.55,2-32.73c0.36-1.27,0.75-2.53,0.98-3.82c1.36-7.75,4.19-10.23,11.88-10.38c1.76-0.04,3.52-0.21,5.76-0.35 c-0.55-3.95-1.21-7.3-1.45-10.68c-0.61-8.67,0.77-16.69,7.39-23.19c2.18-2.14,4.27-4.82,5.25-7.65c2.39-6.88,11.66-9,16.94-8.12 c5.92,0.99,12.15,7.93,12.16,14.12c0.01,9.89-5.19,17.26-12.24,23.68c-2.17,1.97-5.35,4.77-5.17,6.94c0.31,3.78,4.15,5.66,8.08,6.04 c1.82,0.18,3.7,0.37,5.49,0.1c5.62-0.85,8.8,2.17,10.85,6.73C73.38-27.19,78.46-14.9,84.2-2.91c1.52,3.17,4.52,5.91,7.41,8.09 c7.64,5.77,15.57,11.16,23.45,16.61c2.28,1.58,4.64,3.23,7.21,4.14c5.18,1.84,8.09,5.63,9.82,10.46c0.45,1.24,0.19,3.71-0.6,4.18 c-1.06,0.63-3.15,0.27-4.44-0.38c-7.05-3.54-12.84-8.88-19.14-13.5c-3.5-2.57-7.9-4-12.03-5.6c-9.44-3.66-17.73-8.42-22.5-18.09 c-2.43-4.94-6.09-9.27-9.69-14.61c-1.2,10.98-4.46,20.65,1.14,31.19c6.62,12.47,5.89,26.25,1.21,39.49 c-2.52,7.11-6.5,13.74-8.67,20.94c-1.91,6.33-2.2,13.15-3.23,19.75c-0.72,4.63-0.84,9.48-2.36,13.84 c-2.49,7.16-6.67,13.83-5.84,21.82c0.42,4.02,1.29,7.99,2.1,12.8c-3.74-0.49-7.47-0.4-10.67-1.66c-1.33-0.53-2.43-4.11-2.07-6.01 c1.86-9.94,3.89-19.69,0.07-29.74C34.55,108.63,36.19,105.52,36.7,102.84c1.25-8.45,2.51-16.89,3.71-24.9 c-0.83-0.58-0.85-0.59-0.87-0.61c-0.03,0.16-0.07,0.32-0.09,0.48C38.53,86.15,37.62,94.5,36.7,102.84z",
//     "path://M36.7,102.84c-1.17,2.54-2.99,4.98-3.39,7.63c-1.51,9.89-3.31,19.58-1.93,29.95 c0.95,7.15-2.91,14.82-3.57,22.35c-0.64,7.36-0.2,14.86,0.35,22.25c0.12,1.68,2.66,3.17,4.67,5.4c-0.6,0.82-1.5,2.22-2.58,3.48 c-0.96,1.12-1.96,2.35-3.21,3.04c-1.71,0.95-3.71,2.03-5.51,1.9c-1.18-0.08-3.04-2.13-3.16-3.43c-0.44-4.72,0-9.52-0.41-14.25 c-0.94-10.88-2.32-21.72-3.24-32.61c-0.49-5.84-1.63-12.01-0.35-17.54c3.39-14.56,2.8-28.84,0.36-43.4 c-2.71-16.16-1.06-32.4,0.54-48.59c0.91-9.22,4.62-17.36,8.53-25.57c1.32-2.77,1.88-6.84,0.87-9.62C21.89-3.77,18.09-11,14.7-18.38 c-0.56,0.1-1.13,0.21-1.69,0.31C10.17-11.52,6.29-5.2,4.71,1.65C2.05,13.21-4.42,22.3-11.43,31.28c-1.32,1.69-2.51,3.5-3.98,5.04 c-4.85,5.08-3.25,10.98-2.32,16.82c0.25,1.53,0.52,3.06,0.77,4.59c-0.53,0.22-1.07,0.43-1.6,0.65c-1.07-2.09-2.14-4.19-3.28-6.44 c-6.39,2.91-2.67,9.6-5.23,15.16c-1.61-3.31-2.77-5.68-3.93-8.06c0-0.33,0-0.67,0-1c6.96-16.08,14.63-31.9,20.68-48.31 C-5.24-4.07-2.03-18.55,2-32.73c0.36-1.27,0.75-2.53,0.98-3.82c1.36-7.75,4.19-10.23,11.88-10.38c1.76-0.04,3.52-0.21,5.76-0.35 c-0.55-3.95-1.21-7.3-1.45-10.68c-0.61-8.67,0.77-16.69,7.39-23.19c2.18-2.14,4.27-4.82,5.25-7.65c2.39-6.88,11.66-9,16.94-8.12 c5.92,0.99,12.15,7.93,12.16,14.12c0.01,9.89-5.19,17.26-12.24,23.68c-2.17,1.97-5.35,4.77-5.17,6.94c0.31,3.78,4.15,5.66,8.08,6.04 c1.82,0.18,3.7,0.37,5.49,0.1c5.62-0.85,8.8,2.17,10.85,6.73C73.38-27.19,78.46-14.9,84.2-2.91c1.52,3.17,4.52,5.91,7.41,8.09 c7.64,5.77,15.57,11.16,23.45,16.61c2.28,1.58,4.64,3.23,7.21,4.14c5.18,1.84,8.09,5.63,9.82,10.46c0.45,1.24,0.19,3.71-0.6,4.18 c-1.06,0.63-3.15,0.27-4.44-0.38c-7.05-3.54-12.84-8.88-19.14-13.5c-3.5-2.57-7.9-4-12.03-5.6c-9.44-3.66-17.73-8.42-22.5-18.09 c-2.43-4.94-6.09-9.27-9.69-14.61c-1.2,10.98-4.46,20.65,1.14,31.19c6.62,12.47,5.89,26.25,1.21,39.49 c-2.52,7.11-6.5,13.74-8.67,20.94c-1.91,6.33-2.2,13.15-3.23,19.75c-0.72,4.63-0.84,9.48-2.36,13.84 c-2.49,7.16-6.67,13.83-5.84,21.82c0.42,4.02,1.29,7.99,2.1,12.8c-3.74-0.49-7.47-0.4-10.67-1.66c-1.33-0.53-2.43-4.11-2.07-6.01 c1.86-9.94,3.89-19.69,0.07-29.74C34.55,108.63,36.19,105.52,36.7,102.84c1.25-8.45,2.51-16.89,3.71-24.9 c-0.83-0.58-0.85-0.59-0.87-0.61c-0.03,0.16-0.07,0.32-0.09,0.48C38.53,86.15,37.62,94.5,36.7,102.84z",
//     "path://M36.7,102.84c-1.17,2.54-2.99,4.98-3.39,7.63c-1.51,9.89-3.31,19.58-1.93,29.95 c0.95,7.15-2.91,14.82-3.57,22.35c-0.64,7.36-0.2,14.86,0.35,22.25c0.12,1.68,2.66,3.17,4.67,5.4c-0.6,0.82-1.5,2.22-2.58,3.48 c-0.96,1.12-1.96,2.35-3.21,3.04c-1.71,0.95-3.71,2.03-5.51,1.9c-1.18-0.08-3.04-2.13-3.16-3.43c-0.44-4.72,0-9.52-0.41-14.25 c-0.94-10.88-2.32-21.72-3.24-32.61c-0.49-5.84-1.63-12.01-0.35-17.54c3.39-14.56,2.8-28.84,0.36-43.4 c-2.71-16.16-1.06-32.4,0.54-48.59c0.91-9.22,4.62-17.36,8.53-25.57c1.32-2.77,1.88-6.84,0.87-9.62C21.89-3.77,18.09-11,14.7-18.38 c-0.56,0.1-1.13,0.21-1.69,0.31C10.17-11.52,6.29-5.2,4.71,1.65C2.05,13.21-4.42,22.3-11.43,31.28c-1.32,1.69-2.51,3.5-3.98,5.04 c-4.85,5.08-3.25,10.98-2.32,16.82c0.25,1.53,0.52,3.06,0.77,4.59c-0.53,0.22-1.07,0.43-1.6,0.65c-1.07-2.09-2.14-4.19-3.28-6.44 c-6.39,2.91-2.67,9.6-5.23,15.16c-1.61-3.31-2.77-5.68-3.93-8.06c0-0.33,0-0.67,0-1c6.96-16.08,14.63-31.9,20.68-48.31 C-5.24-4.07-2.03-18.55,2-32.73c0.36-1.27,0.75-2.53,0.98-3.82c1.36-7.75,4.19-10.23,11.88-10.38c1.76-0.04,3.52-0.21,5.76-0.35 c-0.55-3.95-1.21-7.3-1.45-10.68c-0.61-8.67,0.77-16.69,7.39-23.19c2.18-2.14,4.27-4.82,5.25-7.65c2.39-6.88,11.66-9,16.94-8.12 c5.92,0.99,12.15,7.93,12.16,14.12c0.01,9.89-5.19,17.26-12.24,23.68c-2.17,1.97-5.35,4.77-5.17,6.94c0.31,3.78,4.15,5.66,8.08,6.04 c1.82,0.18,3.7,0.37,5.49,0.1c5.62-0.85,8.8,2.17,10.85,6.73C73.38-27.19,78.46-14.9,84.2-2.91c1.52,3.17,4.52,5.91,7.41,8.09 c7.64,5.77,15.57,11.16,23.45,16.61c2.28,1.58,4.64,3.23,7.21,4.14c5.18,1.84,8.09,5.63,9.82,10.46c0.45,1.24,0.19,3.71-0.6,4.18 c-1.06,0.63-3.15,0.27-4.44-0.38c-7.05-3.54-12.84-8.88-19.14-13.5c-3.5-2.57-7.9-4-12.03-5.6c-9.44-3.66-17.73-8.42-22.5-18.09 c-2.43-4.94-6.09-9.27-9.69-14.61c-1.2,10.98-4.46,20.65,1.14,31.19c6.62,12.47,5.89,26.25,1.21,39.49 c-2.52,7.11-6.5,13.74-8.67,20.94c-1.91,6.33-2.2,13.15-3.23,19.75c-0.72,4.63-0.84,9.48-2.36,13.84 c-2.49,7.16-6.67,13.83-5.84,21.82c0.42,4.02,1.29,7.99,2.1,12.8c-3.74-0.49-7.47-0.4-10.67-1.66c-1.33-0.53-2.43-4.11-2.07-6.01 c1.86-9.94,3.89-19.69,0.07-29.74C34.55,108.63,36.19,105.52,36.7,102.84c1.25-8.45,2.51-16.89,3.71-24.9 c-0.83-0.58-0.85-0.59-0.87-0.61c-0.03,0.16-0.07,0.32-0.09,0.48C38.53,86.15,37.62,94.5,36.7,102.84z",
//     "path://M36.7,102.84c-1.17,2.54-2.99,4.98-3.39,7.63c-1.51,9.89-3.31,19.58-1.93,29.95 c0.95,7.15-2.91,14.82-3.57,22.35c-0.64,7.36-0.2,14.86,0.35,22.25c0.12,1.68,2.66,3.17,4.67,5.4c-0.6,0.82-1.5,2.22-2.58,3.48 c-0.96,1.12-1.96,2.35-3.21,3.04c-1.71,0.95-3.71,2.03-5.51,1.9c-1.18-0.08-3.04-2.13-3.16-3.43c-0.44-4.72,0-9.52-0.41-14.25 c-0.94-10.88-2.32-21.72-3.24-32.61c-0.49-5.84-1.63-12.01-0.35-17.54c3.39-14.56,2.8-28.84,0.36-43.4 c-2.71-16.16-1.06-32.4,0.54-48.59c0.91-9.22,4.62-17.36,8.53-25.57c1.32-2.77,1.88-6.84,0.87-9.62C21.89-3.77,18.09-11,14.7-18.38 c-0.56,0.1-1.13,0.21-1.69,0.31C10.17-11.52,6.29-5.2,4.71,1.65C2.05,13.21-4.42,22.3-11.43,31.28c-1.32,1.69-2.51,3.5-3.98,5.04 c-4.85,5.08-3.25,10.98-2.32,16.82c0.25,1.53,0.52,3.06,0.77,4.59c-0.53,0.22-1.07,0.43-1.6,0.65c-1.07-2.09-2.14-4.19-3.28-6.44 c-6.39,2.91-2.67,9.6-5.23,15.16c-1.61-3.31-2.77-5.68-3.93-8.06c0-0.33,0-0.67,0-1c6.96-16.08,14.63-31.9,20.68-48.31 C-5.24-4.07-2.03-18.55,2-32.73c0.36-1.27,0.75-2.53,0.98-3.82c1.36-7.75,4.19-10.23,11.88-10.38c1.76-0.04,3.52-0.21,5.76-0.35 c-0.55-3.95-1.21-7.3-1.45-10.68c-0.61-8.67,0.77-16.69,7.39-23.19c2.18-2.14,4.27-4.82,5.25-7.65c2.39-6.88,11.66-9,16.94-8.12 c5.92,0.99,12.15,7.93,12.16,14.12c0.01,9.89-5.19,17.26-12.24,23.68c-2.17,1.97-5.35,4.77-5.17,6.94c0.31,3.78,4.15,5.66,8.08,6.04 c1.82,0.18,3.7,0.37,5.49,0.1c5.62-0.85,8.8,2.17,10.85,6.73C73.38-27.19,78.46-14.9,84.2-2.91c1.52,3.17,4.52,5.91,7.41,8.09 c7.64,5.77,15.57,11.16,23.45,16.61c2.28,1.58,4.64,3.23,7.21,4.14c5.18,1.84,8.09,5.63,9.82,10.46c0.45,1.24,0.19,3.71-0.6,4.18 c-1.06,0.63-3.15,0.27-4.44-0.38c-7.05-3.54-12.84-8.88-19.14-13.5c-3.5-2.57-7.9-4-12.03-5.6c-9.44-3.66-17.73-8.42-22.5-18.09 c-2.43-4.94-6.09-9.27-9.69-14.61c-1.2,10.98-4.46,20.65,1.14,31.19c6.62,12.47,5.89,26.25,1.21,39.49 c-2.52,7.11-6.5,13.74-8.67,20.94c-1.91,6.33-2.2,13.15-3.23,19.75c-0.72,4.63-0.84,9.48-2.36,13.84 c-2.49,7.16-6.67,13.83-5.84,21.82c0.42,4.02,1.29,7.99,2.1,12.8c-3.74-0.49-7.47-0.4-10.67-1.66c-1.33-0.53-2.43-4.11-2.07-6.01 c1.86-9.94,3.89-19.69,0.07-29.74C34.55,108.63,36.19,105.52,36.7,102.84c1.25-8.45,2.51-16.89,3.71-24.9 c-0.83-0.58-0.85-0.59-0.87-0.61c-0.03,0.16-0.07,0.32-0.09,0.48C38.53,86.15,37.62,94.5,36.7,102.84z",
//   ];

//   // Extract audience level peoples choice percentages from the data object
//   const peoplesChoicePercentages =
//     data.percentages["Audience Level Peoples Choice Percentages"];

//   // Prepare the new data for the series based on extracted values
//   const seriesData = [
//     { value: peoplesChoicePercentages["Average Audience"], symbol: symbols[0] },
//     {
//       value: peoplesChoicePercentages["Intermediate Audience"],
//       symbol: symbols[1],
//     },
//     {
//       value: peoplesChoicePercentages["Advanced Audience"],
//       symbol: symbols[2],
//     },
//     { value: peoplesChoicePercentages["Expert Audience"], symbol: symbols[3] },
//   ];

//   // Update the chart options
//   const audienceLevelPeoplesChoicePercentagesOption = {
//     tooltip: {},
//     legend: {
//       data: ["Audience Choices"],
//       selectedMode: "single",
//     },
//     xAxis: {
//       data: [
//         "Average Audience",
//         "Intermediate Audience",
//         "Advanced Audience",
//         "Expert Audience",
//       ],
//       axisTick: { show: false },
//       axisLine: { show: false },
//       axisLabel: { show: true },
//     },
//     yAxis: {
//       max: 100, // Assuming percentages are out of 100
//       offset: 20,
//       splitLine: { show: false },
//     },
//     grid: {
//       top: "center",
//       height: 230,
//     },
//     markLine: {
//       z: -100,
//     },
//     series: [
//       {
//         name: "Audience Choices",
//         type: "pictorialBar",
//         symbolClip: true,
//         symbolBoundingData: 100, // Max value for the percentage
//         label: labelSetting,
//         data: seriesData.map((item) => ({
//           value: item.value,
//           symbol: item.symbol, // Correctly set the symbol for each data item
//         })),
//         markLine: markLineSetting,
//         z: 10,
//       },
//     ],
//   };

//   // Update the chart with the new options
//   audienceLevelPeoplesChoicePercentagesChart.setOption(
//     audienceLevelPeoplesChoicePercentagesOption
//   );
// }

// /*var articleEngagementPopularity = echarts.init(
//   document.getElementById("article-engagement-popularity")
// );

// var articleEngagementPopularityOption = {
//   title: {
//     text: "Funnel",
//   },
//   tooltip: {
//     trigger: "item",
//     formatter: "{a} <br/>{b} : {c}%",
//   },
//   toolbox: {
//     feature: {
//       dataView: { readOnly: false },
//       restore: {},
//       saveAsImage: {},
//     },
//   },
//   legend: {
//     data: ["Show", "Click", "Visit", "Inquiry", "Order"],
//   },
//   series: [
//     {
//       name: "Expected",
//       type: "funnel",
//       left: "10%",
//       width: "80%",
//       label: {
//         formatter: "{b}Expected",
//       },
//       labelLine: {
//         show: false,
//       },
//       itemStyle: {
//         opacity: 0.7,
//       },
//       emphasis: {
//         label: {
//           position: "inside",
//           formatter: "{b}Expected: {c}%",
//         },
//       },
//       data: [
//         { value: 60, name: "Visit" },
//         { value: 40, name: "Inquiry" },
//         { value: 20, name: "Order" },
//         { value: 80, name: "Click" },
//         { value: 100, name: "Show" },
//       ],
//     },
//     {
//       name: "Actual",
//       type: "funnel",
//       left: "10%",
//       width: "80%",
//       maxSize: "80%",
//       label: {
//         position: "inside",
//         formatter: "{c}%",
//         color: "#fff",
//       },
//       itemStyle: {
//         opacity: 0.5,
//         borderColor: "#fff",
//         borderWidth: 2,
//       },
//       emphasis: {
//         label: {
//           position: "inside",
//           formatter: "{b}Actual: {c}%",
//         },
//       },
//       data: [
//         { value: 30, name: "Visit" },
//         { value: 10, name: "Inquiry" },
//         { value: 5, name: "Order" },
//         { value: 50, name: "Click" },
//         { value: 80, name: "Show" },
//       ],
//       // Ensure outer shape will not be over inner shape when hover.
//       z: 100,
//     },
//   ],
// };

// articleEngagementPopularityOption &&
//   articleEngagementPopularity.setOption(articleEngagementPopularityOption);
// function updateArticleEngagementPopularityChart(data) {
//   // Extract the relevant engagement data from the provided object
//   const popularity =
//     data.percentages["Article Sentiment Popularity Breakdown"].Popularity;
//   const likability =
//     data.percentages["Article Sentiment Likability Breakdown"].likability;
//   const engagementPercentage =
//     data.percentages["Article Sentiment Engagement Breakdown"]
//       .engagement_percentage;

//   // Update values based on the sentiment analysis
//   const expectedData = [
//     { value: 100, name: "Show" },
//     { value: 80, name: "Click" },
//     { value: 60, name: "Visit" }, // Placeholder values based on expectations
//     { value: 40, name: "Inquiry" },
//     { value: 20, name: "Order" },
//   ];

//   const actualData = [
//     { value: 80, name: "Show" }, // Placeholder based on your data (e.g., 80% actual shows)
//     { value: 50, name: "Click" }, // Placeholder based on your data
//     { value: 30, name: "Visit" }, // Placeholder; could relate to engagements
//     { value: 10, name: "Inquiry" },
//     { value: engagementPercentage, name: "Order" }, // Using the engagement percentage
//   ];

//   // Create or Update the chart options using the newly set data
//   articleEngagementPopularityOption.series[0].data = expectedData;
//   articleEngagementPopularityOption.series[1].data = actualData;

//   // Update the chart with the new options
//   articleEngagementPopularity.setOption(articleEngagementPopularityOption);
// }*/

// //==============================================================================================//
// //================================Fetched user by id===========================================//

// const userCatergoricalReputationData = echarts.init(
//   document.getElementById("user-catergorical-reputation-data")
// );

// let userCatergoricalReputationDataOption = {
//   title: {
//     text: "User Categorical Reputation Data",
//     subtext: "MPXR Code",
//     left: "center",
//   },
//   tooltip: {
//     trigger: "item",
//     formatter: "{a} <br/>{b} : {c} ({d}%)",
//   },
//   legend: {
//     type: "scroll",
//     orient: "vertical",
//     right: 10,
//     top: 20,
//     bottom: 20,
//     data: [], // Initially empty
//   },
//   series: [
//     {
//       name: "User Reputation per Category",
//       type: "pie",
//       radius: "55%",
//       center: ["40%", "50%"],
//       data: [], // Initially empty
//       emphasis: {
//         itemStyle: {
//           shadowBlur: 10,
//           shadowOffsetX: 0,
//           shadowColor: "rgba(0, 0, 0, 0.5)",
//         },
//       },
//     },
//   ],
// };
// userCatergoricalReputationDataOption &&
//   userCatergoricalReputationData.setOption(
//     userCatergoricalReputationDataOption
//   );

// function updateUserCatergoricalReputationData(data) {
//   if (data.categories && Array.isArray(data.categories)) {
//     const seriesData = data.categories.map((category) => ({
//       name: category.category,
//       value: Math.round(category.rep * 100000), // Scaling the reputation for visualization
//     }));

//     userCatergoricalReputationDataOption.series[0].data = seriesData;
//     userCatergoricalReputationDataOption.legend.data = seriesData.map(
//       (item) => item.name
//     );

//     userCatergoricalReputationData.setOption(
//       userCatergoricalReputationDataOption
//     );
//   } else {
//     console.error("Invalid data object: categories not found or malformed.");
//   }
// }

// var networkAnalysisReputationScorePercentageChart = echarts.init(
//   document.getElementById("network-analysis-reputation-score-percentage")
// );

// var networkAnalysisReputationScorePercentageOption = {
//   polar: {},
//   angleAxis: {
//     type: "category",
//     data: [
//       "Expert Audience",
//       "Advanced Audience",
//       "Intermediate Audience",
//       "Average Audience",
//     ],
//     z: 10,
//   },
//   radiusAxis: {
//     type: "value",
//     min: 0,
//   },
//   series: [
//     {
//       name: "Positive MPXR Score",
//       type: "bar",
//       data: [], // To be updated with positive scores
//       coordinateSystem: "polar",
//       itemStyle: {
//         color: "blue",
//       },
//       emphasis: {
//         focus: "series",
//       },
//     },
//     {
//       name: "Negative MPXR Score",
//       type: "bar",
//       data: [], // To be updated with negative scores
//       coordinateSystem: "polar",
//       itemStyle: {
//         color: "green",
//       },
//       emphasis: {
//         focus: "series",
//       },
//     },
//   ],
//   legend: {
//     show: true,
//     data: ["Positive MPXR Score", "Negative MPXR Score"],
//   },
// };

// // Initializing with empty data
// networkAnalysisReputationScorePercentageChart.setOption(
//   networkAnalysisReputationScorePercentageOption
// );

// // Function to update the chart
// function updateNetworkAnalysisChart(data) {
//   if (data && data["Network Breakdown"]) {
//     const positiveScores =
//       data["Network Breakdown"]["Source of Positive MPXR Score"];
//     const negativeScores =
//       data["Network Breakdown"]["Source of Negative MPXR Score"];

//     // Extracting scores
//     const positiveData = [
//       positiveScores["Expert Audience"],
//       positiveScores["Advanced Audience"],
//       positiveScores["Intermediate Audience"],
//       positiveScores["Average Audience"],
//     ];

//     const negativeData = [
//       negativeScores["Expert Audience"],
//       negativeScores["Advanced Audience"],
//       negativeScores["Intermediate Audience"],
//       negativeScores["Average Audience"],
//     ];

//     // Update the series data
//     networkAnalysisReputationScorePercentageOption.series[0].data =
//       positiveData; // Positive scores
//     networkAnalysisReputationScorePercentageOption.series[1].data =
//       negativeData; // Negative scores

//     // Set the updated option
//     networkAnalysisReputationScorePercentageChart.setOption(
//       networkAnalysisReputationScorePercentageOption
//     );
//   } else {
//     console.error("Invalid data provided for the chart update.");
//   }
// }

// var ROOT_PATH = "https://echarts.apache.org/examples";

// var userExpertiseLevelAndUserNatureForUserChart = echarts.init(
//   document.getElementById("user-expertise-level-and-user-nature-for-user")
// );
// var userExpertiseLevelAndUserNatureForUserOption;

// userExpertiseLevelAndUserNatureForUserChart.showLoading();

// // Function to update the chart
// function updateUserExpertiseLevelAndUserNatureForUserChart(data) {
//   userExpertiseLevelAndUserNatureForUserChart.hideLoading();

//   // Construct the tree structure from the provided user data
//   var updatedTreeData = {
//     name: "User Data",
//     children: [
//       {
//         name: "User Nature",
//         children: [
//           {
//             name: "Type: " + data["User Nature"]["user nature"],
//           },
//           {
//             name:
//               "Published Articles: " +
//               data["User Nature"]["number of published articles"],
//           },
//         ],
//       },
//       {
//         name: "User Level",
//         children: [
//           {
//             name: data["User Level"],
//           },
//         ],
//       },
//     ],
//   };

//   // Set option for the chart with the updated tree data
//   userExpertiseLevelAndUserNatureForUserChart.setOption(
//     (userExpertiseLevelAndUserNatureForUserOption = {
//       tooltip: {
//         trigger: "item",
//         triggerOn: "mousemove",
//       },
//       series: [
//         {
//           type: "tree",
//           data: [updatedTreeData], // Using the new tree data
//           top: "1%",
//           left: "7%",
//           bottom: "1%",
//           right: "20%",
//           symbolSize: 7,
//           label: {
//             position: "left",
//             verticalAlign: "middle",
//             align: "right",
//             fontSize: 9,
//           },
//           leaves: {
//             label: {
//               position: "right",
//               verticalAlign: "middle",
//               align: "left",
//             },
//           },
//           emphasis: {
//             focus: "descendant",
//           },
//           expandAndCollapse: true,
//           animationDuration: 550,
//           animationDurationUpdate: 750,
//         },
//       ],
//     })
//   );
// }

// var ROOT_PATH = "https://echarts.apache.org/examples";

// var interactionBreakdownInNumbersUsersFavoriteChart = echarts.init(
//   document.getElementById("interaction-breakdown-in-numbers-users-favorite")
// );

// var interactionBreakdownInNumbersUsersFavoriteOption;

// interactionBreakdownInNumbersUsersFavoriteChart.showLoading();
// $.get(ROOT_PATH + "/data/asset/data/flare.json", function (data) {
//   interactionBreakdownInNumbersUsersFavoriteChart.hideLoading();
//   data.children.forEach(function (datum, index) {
//     index % 2 === 0 && (datum.collapsed = true);
//   });
//   interactionBreakdownInNumbersUsersFavoriteChart.setOption(
//     (interactionBreakdownInNumbersUsersFavoriteOption = {
//       tooltip: {
//         trigger: "item",
//         triggerOn: "mousemove",
//       },
//       series: [
//         {
//           type: "tree",
//           data: [data],
//           top: "1%",
//           left: "7%",
//           bottom: "1%",
//           right: "20%",
//           symbolSize: 7,
//           label: {
//             position: "left",
//             verticalAlign: "middle",
//             align: "right",
//             fontSize: 9,
//           },
//           leaves: {
//             label: {
//               position: "right",
//               verticalAlign: "middle",
//               align: "left",
//             },
//           },
//           emphasis: {
//             focus: "descendant",
//           },
//           expandAndCollapse: true,
//           animationDuration: 550,
//           animationDurationUpdate: 750,
//         },
//       ],
//     })
//   );
// });

// // Function to update the chart with interaction data
// function updateinteractionBreakdownInNumbersUsersFavoriteChart(data) {
//   // Structure the data according to the user interaction object
//   const treeData = {
//     name: "User Interaction Breakdown",
//     children: [
//       {
//         name: "User Like Interaction",
//         children: [
//           {
//             name:
//               "Thumbs Up: " +
//               data["User Like Interaction By Type Breakdown"][
//               "User Like Interaction By Type"
//               ]["Percentages"]["Percentage of Thumbs Up"],
//           },
//           {
//             name:
//               "Thumbs Down: " +
//               data["User Like Interaction By Type Breakdown"][
//               "User Like Interaction By Type"
//               ]["Percentages"]["Percentage of Thumbs Down"],
//           },
//           {
//             name: "Breakdown by Quality",
//             children: [
//               {
//                 name:
//                   "Thumbs Up Best Quality: " +
//                   data["User Like Interaction By Type Breakdown"][
//                   "User Like Interaction By Type"
//                   ]["Breakdown by Quality"]["Percentage of Thumbs Up"][
//                   "No. of Best Quality Int."
//                   ],
//               },
//               {
//                 name:
//                   "Thumbs Up Low Quality: " +
//                   data["User Like Interaction By Type Breakdown"][
//                   "User Like Interaction By Type"
//                   ]["Breakdown by Quality"]["Percentage of Thumbs Up"][
//                   "No. of Low Quality Int."
//                   ],
//               },
//               {
//                 name:
//                   "Thumbs Down Best Quality: " +
//                   data["User Like Interaction By Type Breakdown"][
//                   "User Like Interaction By Type"
//                   ]["Breakdown by Quality"]["Percentage of Thumbs Down"][
//                   "No. of Best Quality Int."
//                   ],
//               },
//               {
//                 name:
//                   "Thumbs Down Low Quality: " +
//                   data["User Like Interaction By Type Breakdown"][
//                   "User Like Interaction By Type"
//                   ]["Breakdown by Quality"]["Percentage of Thumbs Down"][
//                   "No. of Low Quality Int."
//                   ],
//               },
//             ],
//           },
//         ],
//       },
//       {
//         name: "User Interaction by Quality",
//         children: [
//           {
//             name:
//               "Best Quality Interactions: " +
//               data["User Interaction by Quality Breakdown"][
//               "User Interaction by Quality"
//               ]["No. of Best Quality Int."],
//           },
//           {
//             name:
//               "Good Quality Interactions: " +
//               data["User Interaction by Quality Breakdown"][
//               "User Interaction by Quality"
//               ]["No. of Good Quality Int."],
//           },
//           {
//             name:
//               "Average Quality Interactions: " +
//               data["User Interaction by Quality Breakdown"][
//               "User Interaction by Quality"
//               ]["No. of Average Quality Int."],
//           },
//           {
//             name:
//               "Low Quality Interactions: " +
//               data["User Interaction by Quality Breakdown"][
//               "User Interaction by Quality"
//               ]["No. of Low Quality Int."],
//           },
//         ],
//       },
//       {
//         name: "User Comment Interaction Percentages",
//         children: [
//           {
//             name:
//               "Percentage of All Best: " +
//               data["User Comment Interaction By Type Percentage Breakdown"][
//               "User Comment Interaction Percentages Breakdown"
//               ]["Percentage of Comment"]["Percentage of All Best"],
//           },
//           {
//             name:
//               "Percentage of All Good: " +
//               data["User Comment Interaction By Type Percentage Breakdown"][
//               "User Comment Interaction Percentages Breakdown"
//               ]["Percentage of Comment"]["Percentage of All Good"],
//           },
//           // Add other comment interaction percentages here...
//         ],
//       },
//       {
//         name: "User Emoji Interaction Breakdown",
//         children: [
//           {
//             name:
//               "Positive Reactions - This Content is Perfect!: " +
//               data["User Emoji Interaction By Type Percentage Breakdown"][
//               "Emoji Reaction Breakdown"
//               ]["Positive Reaction"]["This content is perfect!"],
//           },
//           {
//             name:
//               "Negative Reactions - This Content is Useless: " +
//               data["User Emoji Interaction By Type Percentage Breakdown"][
//               "Emoji Reaction Breakdown"
//               ]["Negative Reaction"][
//               "This content is useless and mostly wrong"
//               ],
//           },
//           // Add other emoji interaction breakdowns here...
//         ],
//       },
//     ],
//   };

//   interactionBreakdownInNumbersUsersFavoriteChart.setOption({
//     series: [
//       {
//         type: "tree",
//         data: [treeData],
//         top: "1%",
//         left: "7%",
//         bottom: "1%",
//         right: "20%",
//         symbolSize: 7,
//         label: {
//           position: "left",
//           verticalAlign: "middle",
//           align: "right",
//           fontSize: 9,
//         },
//         leaves: {
//           label: {
//             position: "right",
//             verticalAlign: "middle",
//             align: "left",
//           },
//         },
//         emphasis: {
//           focus: "descendant",
//         },
//         expandAndCollapse: true,
//         animationDuration: 550,
//         animationDurationUpdate: 750,
//       },
//     ],
//   });
// }
// //not working

// /*var MPXRFromInteractionPercentageUserChart = echarts.init(
//   document.getElementById("MPXR-from-interaction-percentage-user")
// );
// var MPXRFromInteractionPercentageUserOption;

// MPXRFromInteractionPercentageUserOption = {
//   title: {
//     text: "Referer of a Website",
//     subtext: "Fake Data",
//     left: "center",
//   },
//   tooltip: {
//     trigger: "item",
//   },
//   legend: {
//     orient: "vertical",
//     left: "left",
//   },
//   series: [
//     {
//       name: "Access From",
//       type: "pie",
//       radius: "50%",
//       data: [
//         { value: 1048, name: "Search Engine" },
//         { value: 735, name: "Direct" },
//         { value: 580, name: "Email" },
//         { value: 484, name: "Union Ads" },
//         { value: 300, name: "Video Ads" },
//       ],
//       emphasis: {
//         itemStyle: {
//           shadowBlur: 10,
//           shadowOffsetX: 0,
//           shadowColor: "rgba(0, 0, 0, 0.5)",
//         },
//       },
//     },
//   ],
// };

// MPXRFromInteractionPercentageUserOption &&
//   MPXRFromInteractionPercentageUserChart.setOption(
//     MPXRFromInteractionPercentageUserOption
//   );*/

// var MPXRFromInteractionPercentageUserChart = echarts.init(
//   document.querySelector("#MPXR-from-interaction-percentage-user")
// );

// // Define the initial chart options
// var MPXRFromInteractionPercentageUserOption = {
//   title: {
//     text: "Referer of a Website",
//     subtext: "Fake Data",
//     left: "center",
//   },
//   tooltip: {
//     trigger: "item",
//   },
//   legend: {
//     orient: "vertical",
//     left: "left",
//   },
//   series: [
//     {
//       name: "Access From",
//       type: "pie",
//       radius: "50%",
//       data: [], // This will be updated later
//       emphasis: {
//         itemStyle: {
//           shadowBlur: 10,
//           shadowOffsetX: 0,
//           shadowColor: "rgba(0, 0, 0, 0.5)",
//         },
//       },
//     },
//   ],
// };

// // Set the initial option
// MPXRFromInteractionPercentageUserOption &&
//   MPXRFromInteractionPercentageUserChart.setOption(
//     MPXRFromInteractionPercentageUserOption
//   );

// // Function to update chart data based on new data
// function updateMPXRFromInteractionPercentageUserChart(data) {
//   // Extract values from the provided object
//   const interactionData =
//     data["Total User Interaction Breakdown"][
//     "Total User Percentage of Interactions"
//     ];

//   // Prepare data for the pie chart
//   const updatedData = [
//     { value: interactionData.time_spent, name: "Time Spent" },
//     { value: interactionData.peoples_choise, name: "People's Choice" },
//     { value: interactionData.comment, name: "Comment" },
//     { value: interactionData.like, name: "Like" },
//     { value: interactionData.dislike, name: "Dislike" },
//   ];

//   // Update the pie chart with new data
//   MPXRFromInteractionPercentageUserChart.setOption({
//     series: [
//       {
//         data: updatedData,
//       },
//     ],
//   });
// }
